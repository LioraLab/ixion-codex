...
3. 스코프 확장(김에 수정) 금지.
4. 검증 게이트(상황별):
   - 코드/설정 변경이 있으면: 검증 **2개**를 기본값으로 실행한다(가장 빠른 2개).
   - 읽기 전용 분석/문서 작업이면: 검증은 **0-1개**(필요할 때만).
   - 검증 커맨드가 불명확하면 `$ixion-doctor`로 후보를 먼저 뽑는다.
5. 결정/리스크는 짧게 남긴다.
...
3. 구현한다.
4. 검증한다.
   - 코드/설정 변경이면: 가능한 한 빠른 검증 2개(예: test + lint/typecheck, 또는 build + lint)
   - 읽기 전용 분석/문서만이면: 0-1개(필요할 때만)
5. 결과를 근거 중심으로 보고한다.
