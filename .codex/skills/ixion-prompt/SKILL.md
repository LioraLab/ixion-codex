...
- 도메인 프리셋: `$ixion-web`, `$ixion-app`, `$ixion-software`, `$ixion-cv`, `$ixion-shopify`
  - 도메인이 명확하고, 체크리스트(보안/접근성/성능/호환성 등)를 기본으로 깔고 갈 때.
...
- 검증 루프: `$ixion-verify`
  - 테스트/빌드/린트/타입체크를 최소 1개 실행하고 근거로만 보고하게 할 때.
- 코드 리뷰: `$ixion-review`
...
검증:
- 검증 커맨드가 불명확하면 먼저 `$ixion-doctor`로 추천 1개를 뽑아 실행
- 최소 1개(테스트/빌드/린트/타입체크/스모크) 실행 결과로만 보고
...
검증:
- 완료 선언 전 `$ixion-verify` 기준으로 최소 1개 실행(테스트/빌드/린트/타입체크/스모크)
...
목적:
- 변경 사항에 대해 최소 1개 검증을 실행하고, 결과로만 보고
...
- (검증) 테스트/빌드/린트/타입체크 중 최소 1개 PASS
...
검증:
- `docker build` 또는 프로젝트 빌드/테스트 중 최소 1개 실행 결과로만 보고
...
- (검증) `flutter test` 또는 `npm test` 등 최소 1개 PASS
